...
function renderHolidayList() {
  ...
  els.holidayList.innerHTML = holidays.map((holiday) => `
    <article class="holiday-row">
      ...
      <button type="button" class="toolbtn sm ghost danger holiday-row-delete" data-action="delete-holiday" data-holiday-id="${escAttr(holiday.id)}" title="${escAttr(`删除假日 ${holiday.name}`)}" aria-label="删除假日 ${escAttr(holiday.name)}">
...
function renderDataStatus() {
  ...
  els.dataStatus.dataset.tone = state.ui.readOnly ? "error" : "info";
  els.dataStatus.textContent = state.ui.readOnly
    ? "当前浏览器存储不可写，修改后可能无法保留。建议尽快导出备份。"
    : "当前数据默认保存在浏览器缓存中；如需迁移或备份，请手动导入 / 导出 JSON。";
}

function syncImportedDb(nextDb, fileName) {
  state.db = validateAndNormalize(nextDb);
  const saved = saveDb();
  renderAll();
  closeHashModal();
  if (saved) {
    toast("导入成功", `已从 ${fileName} 恢复当前数据。`, false);
  } else {
    toast("导入成功", `已读取 ${fileName}，但浏览器缓存不可写，请立即导出备份。`, true);
  }
}

function triggerDataImportPicker() {
  if (!els.dataImportInput) {
    toast("导入失败", "当前页面缺少导入控件。", true);
    return;
  }
...
async function importDbFromFile(file) {
  ...
  try {
    ...
    const normalizedText = safeString(text).replace(/^\uFEFF/, "").trim();
    if (!normalizedText) {
      toast("导入失败", "所选文件为空。", true);
      return false;
    }
    const parsed = JSON.parse(normalizedText);
    syncImportedDb(parsed, safeString(file.name) || "JSON 文件");
    return true;
  } catch (_error) {
    toast("导入失败", "文件内容不是合法的 JSON。", true);
    return false;
  }
}
...
function buildExportFileName() {
  return `${EXPORT_FILE_PREFIX}-${formatYmd(new Date())}.json`;
}

function exportDbToFile() {
  const payload = JSON.stringify(state.db || defaultDb(), null, 2);
  downloadTextFile(buildExportFileName(), payload, "application/json;charset=utf-8");
  toast("已导出", "当前数据已导出为 JSON 文件。", false);
}

function saveDb() {
  const payload = JSON.stringify(state.db || defaultDb());
  const ok = safeWriteStorage(STORAGE_KEY, payload);
  if (!ok) {
    state.ui.readOnly = true;
    renderDataStatus();
    toast("保存失败", "浏览器缓存不可写，修改可能无法保留。", true);
    return false;
  }
